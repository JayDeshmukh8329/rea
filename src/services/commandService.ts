export function processCommand(command: string, name: string = "Jay"): {
  action: string;
  url?: string;
  ...
  if (ytMatch) {
    ...
    return {
      action: `Playing ${ytMatch[1]} on YouTube. Don't judge my music taste, ${name}.`,
  ...
  if (spotifyMatch) {
    ...
    return {
      action: `Searching ${spotifyMatch[1]} on Spotify. Hope it's a banger, ${name}.`,
  ...
  if (waMatch) {
    ...
    return {
      action: `Sending your message. Let's hope they reply, ${name}.`,

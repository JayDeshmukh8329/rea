...
export class LiveSessionManager {
  ...
  public onMessage: (sender: "user" | "era", text: string) => void = () => {};
  public onCommand: (url: string) => void = () => {};
  public onVolumeChange: (volume: number) => void = () => {};
  public onError: (message: string, isFatal: boolean) => void = () => {};
  ...
  async start() {
    try {
      ...
      this.processor.onaudioprocess = (e) => {
        ...
        // Report volume (0 to 1)
        const avg = sum / pcm16.length;
        const volume = Math.min(1, avg / 4000); // Normalize heuristic
      ...
      this.sessionPromise = this.ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: getSystemInstruction(this.userProfile),
        ...
        callbacks: {
          ...
          onerror: (err) => {
            ...
            this.onError("The connection dropped. Probably your internet's fault.", true);
            this.stop();
          }
        }
      });
      ...
      let message = "I can't hear you. Check your mic or stop being so quiet!";
      if (error.name === "NotAllowedError") {
        message = "You didn't give me mic access. How am I supposed to listen?";
      } else if (error.message?.includes("network")) {
        message = "My connection is acting up. Even I have bad days.";
      }
  ...
  private playAudioChunk(base64Data: string) {
    ...
    try {
      ...
      const volume = Math.min(1, (sum / buffer.length) * 10); // Amplify for visualization
      this.onVolumeChange(volume);
